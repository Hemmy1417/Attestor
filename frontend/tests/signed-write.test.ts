/**
 * Repository-level proof that Attestor contract writes are SIGNED by the
 * connected wallet.
 *
 ...
 *
 * and useAttestorContract (lib/hooks/useAttestor.ts) injects that client into
 * the Attestor wrapper. These tests pin the three properties that make writes
 * wallet-signed:
 *
...
import Attestor from "../lib/contracts/attestor";
...
describe("signed writes via the wallet-context client", () => {
  it("routes writes through the injected provider-backed client", async () => {
    ...
    const attestor = new Attestor(CONTRACT, walletClient);

    const { txHash } = await attestor.cancelJob("1");
  ...
  it("passes the payable value through the injected client", async () => {
    ...
    const attestor = new Attestor(CONTRACT, walletClient);
    const bountyWei = 10n ** 18n;

    await attestor.postJob({
      title: "Test job",
      brief: "A test brief long enough to pass validation.",
      proofCriteria: "Criteria long enough to pass validation.",
      workerAddress: "",
      maxAttempts: 3,
      bountyWei,
    ...
    expect(walletClient.writeContract).toHaveBeenCalledWith(
      expect.objectContaining({ functionName: "post_job", value: bountyWei }),
    );
  });

  it("refuses to write when no wallet client is connected (no unsigned fallback)", async () => {
    const attestor = new Attestor(CONTRACT); // read-only: bare RPC client, no signer
    await expect(attestor.cancelJob("1")).rejects.toThrow(/connect a wallet/i);
  });
